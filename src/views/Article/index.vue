<template>
  <div class="article-container">
    ...
    <!-- 给评论回复的弹出层 -->
    <van-popup
      v-model="isrReplayShow"
      position="bottom"
      :style="{ height: '100%' }"
      ><ReplayComment
        :comment="comment"
        @close="isrReplayShow = false"
        v-if="isrReplayShow"
      ></ReplayComment
    ></van-popup>
  </div>
...
<script>
import ReplayComment from './commpnoents/ReplayComment.vue'
...
export default {
  name: 'ArticleIndex',
  components: { AddComment, ReplayComment },
  props: {
    article_id: {
      type: [Number, String],
      required: true
